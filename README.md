...
`ask.html`에서 AI 답변마다 "💾 저장" 버튼을 누르면 `notes.html`에서 언제든 다시 볼 수 있습니다. 별도 서버·데이터베이스 없이 브라우저의 로컬 저장소(localStorage)에만 저장되므로, 같은 브라우저·기기에서만 보이고 다른 기기와는 동기화되지 않습니다. `notes.html`에서 노트별 삭제 및 전체 삭제가 가능합니다.
